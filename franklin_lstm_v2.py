...
df.drop('subscription_orders', axis=1, inplace=True)    
...
df_complete[['order_volume', 'avg_paid', 'daily_revenue', 'item_cost']] = df_complete[['order_volume', 'avg_paid', 'daily_revenue', 'item_cost']].fillna(0)
...
columns_to_reshape = ['sku_group', 'item_cost', 'avg_paid', 'daily_revenue', 'order_volume', 'accessories', 'dry', 'other', 'sample', 'supplements', 'treat', 'wet']
...
model = Sequential()
model.add(LSTM(30, activation='relu', return_sequences=True, kernel_regularizer=regularizers.l2(0.01), input_shape=(window_size, X_train.shape[1])))
model.add(Dropout(0.2))                     # disables ndes with 0.4 chance to increase burden on others and reduce overfitting
model.add(LSTM(30, activation='relu', return_sequences=True, kernel_regularizer=regularizers.l2(0.01)))
model.add(Dropout(0.2))                     # disables ndes with 0.4 chance to increase burden on others and reduce overfitting
